...
while obj_diff > tol %l_diff > tol || lambda_diff > tol
    ...
    subject to
        for j = 1:J
            for i = 1:(d-2)
                ...
                -dl((j-1)*d + i) + log( w*exp(dl((j-1)*d + i+1) + dl((j-1)*d + i)) + (1-w) ) <= 0;

                %constraint (3.2) - beta Lipschitz derivatives approximation
                dl((j-1)*d + i+1)*int_lengths(i)/int_lengths(i+1) - dl((j-1)*d + i) >= -beta*int_lengths(i)^2;
            end
